...
export default function Relatorio() {
  ...
  useEffect(() => {
    ...
    capsData.forEach((item) => {
      ...
      if (date.getFullYear() === tampinhasYear) {
        ...
        mesesMap[month] += item.quantidadeKg;
      }
    });
  ...
  // ✅ PROCESSAR CARDS (ANIMAIS + TAMPINHAS)
  useEffect(() => {
    if (!animalsData.length || !capsData.length) return;
    ...
    animalsData.forEach((item) => {
      ...
      if (
        ...
        if (item.tipoAnimal === 'gato') {
          totalCats += item.quantidade;
        }

        if (item.tipoAnimal === 'cachorro') {
          totalDogs += item.quantidade;
        }
      }
    });
